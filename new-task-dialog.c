...
GtkWidget *sct_gtk_new_task_dialog_new(
    ...
    gtk_cell_layout_pack_start(
            GTK_CELL_LAYOUT(ntds->project_combo_box), cell, TRUE);
    gtk_cell_layout_set_attributes(
            GTK_CELL_LAYOUT(ntds->project_combo_box), cell, "text", 0, NULL);
    ...
    label = gtk_label_new_with_mnemonic(_("_Project"));
    gtk_label_set_mnemonic_widget(GTK_LABEL(label), ntds->project_combo_box);
    gtk_table_attach(
            GTK_TABLE(table), label, 0, 1, 1, 2, 
            GTK_EXPAND | GTK_FILL, GTK_FILL, 5, 5);

    gtk_table_attach(
            GTK_TABLE(table), ntds->project_combo_box, 1, 2, 1, 2, 
            GTK_EXPAND | GTK_FILL, GTK_FILL, 5, 5);
    ...
    gtk_table_attach(
            GTK_TABLE(table), ntds->scheduled_for_entry, 1, 2, 2, 3, 
            GTK_EXPAND | GTK_FILL, GTK_FILL, 5, 5);
